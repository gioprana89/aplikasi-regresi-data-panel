...
modul_literature_review_server <- function(input, output, session) {
  ...
  output$buka_pemilihan_informasi <- renderUI({
      ...
      checkboxGroupInput(session$ns("terpilih_variabel"), 
                         label="Select Variables:", choices = c(nama_variabel()), selected=c("Judul", "Tahun", "Jurnal", "Link Artikel", "Variabel","Statistik PLS-SEM","Software yang Digunakan" ), inline = TRUE)
